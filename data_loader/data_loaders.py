...
class DDIGraphDataLoader(BaseDataLoader):
    def __init__(self, logger, data_dir, batch_size, task='train', shuffle=True, validation_split=0.1, test_split=0.2, num_workers=1):
        self.logger = logger
        self.data_dir = data_dir
        self.task = task
        if self.task == 'All':
            self.save_dir = join(self.data_dir, 'processed-All')
        else:
            self.save_dir = join(self.data_dir, 'processed-COVID19')
    ...
    def _load_data(self):
        ...
        if exists(join(self.save_dir, 'dpi.csv')):
            ddi_df = pd.read_csv(join(self.save_dir, 'ddi.csv'))
            dpi_df = pd.read_csv(join(self.save_dir, 'dpi.csv'))
            ppi_df = pd.read_csv(join(self.save_dir, 'ppi.csv'))
            return ddi_df, dpi_df, ppi_df
        else:
            os.makedirs(self.save_dir, exist_ok=True)
        
        self.ddi_df = pd.read_csv(join(self.data_dir, 'data_train', 'ddi.csv'), 
            usecols=['drug1', 'drug2', 'label'])
        self.ddi_df.columns = ['drug_1', 'drug_2', 'label']

        self.dpi_df = pd.read_csv(join(self.data_dir, 'data_train', 'dpi.csv'),
            usecols=['drugbank_id', 'entrez_id'])
        self.dpi_df.columns = ['drug', 'protein']

        if self.task == 'All':
            self.logger.info('Loading tcm target data from {}'.format('tcm-All/formula2target.csv'))
            self.tcm_target_df = pd.read_csv(join(self.data_dir, 'tcm-All', 'formula2target.csv'),
                                             usecols=['formula_idx', 'target'])
        else:
            self.logger.info('Loading COVID-19 specific tcm target from {}'.format('tcm-COVID19/tcm2target.csv'))
            self.tcm_target_df = pd.read_csv(join(self.data_dir, 'tcm-COVID19', 'tcm2target.csv'),
        ...
        self.ppi_df = pd.read_excel(join(self.data_dir, 'data_train', 'ppi.xlsx'),
    ...
    def _node_map(self):
        ...
        # label: 1 for positive, 0 for known, -1 for negative
        self.ddi_df['label'] = self.ddi_df['label'].apply(lambda x: 1 if x == -1 else 0)
        ...
        all_node_map_df.to_csv(join(self.save_dir, 'node_map.csv'), index=False)

        self.ddi_df.to_csv(join(self.save_dir, 'ddi.csv'), index=False)
        self.dpi_df.to_csv(join(self.save_dir, 'dpi.csv'), index=False)
        self.ppi_df.to_csv(join(self.save_dir, 'ppi.csv'), index=False)
