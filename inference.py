...
def main(config):
    ...
    if os.path.exists('data/processed-All/ddi_to_predict.csv'):
        data_inference_df = pd.read_csv('data/processed-All/ddi_to_predict.csv')
    else:
        logger.info('Creating the drug pairs to predict...')
        dpi_df = pd.read_csv('data/processed-All/dpi.csv')
        drug_index_list = list(set(dpi_df['drug']))
        from itertools import combinations
        ddi_to_predict_list = combinations(drug_index_list, 2)
        data_inference_df = pd.DataFrame(ddi_to_predict_list, columns=['drug1', 'drug2'])
        data_inference_df.to_csv('data/processed-All/ddi_to_predict.csv', index=False)

    node_map_df = pd.read_csv('data/processed-All/node_map.csv')
    ...
    data_loader_inference = torch.utils.data.DataLoader(ddiDataset_inference, batch_size=10240, shuffle=False, num_workers=0)
    ...
    resume = 'saved/models/DDI-GCN-Train/0613_165911/model_best.pth'
    ...
    for (drug1, drug2, label) in tqdm(data_loader_inference):
        drug1 = drug1.cuda()
        drug2 = drug2.cuda()
        label = label.cuda()
        with torch.no_grad():
    ...
    result_df.to_csv('data/tcm-All/ddi_predicted.csv', index=False)
